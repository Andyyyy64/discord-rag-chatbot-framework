...
- 類似度順を保持したまま上位N件に絞り込み（`TOP_CANDIDATES_LIMIT` で設定可能、デフォルト: 50）
- Map を使用して効率的にマッピング
...
- デフォルトTop-20を選択（`RERANK_TOPK` で変更可能）
- リランク失敗時はベクトル検索の結果をそのまま使用

### 5. プロンプト構築

**実装場所**: `src/domain/chat/chat-service.ts` (180-218行目)

**処理内容**:
- 選択されたウィンドウを「過去の会話ログ」としてフォーマット
- 各ウィンドウに日時と別チャンネル情報を付与
- システムプロンプトで推論・要約・まとめを明示的に許可
- 「会話アシスタント」としての役割を定義
- 自然な言い回しでの回答を促すスタイル指示
...
4. **候補の絞り込み**
   - 200件 → 類似度上位50件（`TOP_CANDIDATES_LIMIT`で設定可能）

5. **再ランク（オプション）**
   - 50件 → Cohere Rerank → Top-20（`RERANK_TOPK`で設定可能）

6. **プロンプト構築 + LLM回答生成**
   - コンテキスト(20件) + 質問 → Gemini → 回答
...
| `RERANK_TOPK` | `20` | 再ランク後のTop-K件数（最終的にプロンプトに含める件数） |
| `TOP_CANDIDATES_LIMIT` | `50` | ベクトル検索後の候補絞り込み件数 |
...
**対処**:
- 候補数を増やす: `.env` で `TOP_CANDIDATES_LIMIT=100` などに設定
- 最終選択数を増やす: `.env` で `RERANK_TOPK=30` などに設定
- 再ランクを有効化: `.env` で `RERANK_PROVIDER=cohere` を設定
- ベクトル検索の候補数を増やす: コード内の `VECTOR_LIMIT` を 200 → 500 などに変更
